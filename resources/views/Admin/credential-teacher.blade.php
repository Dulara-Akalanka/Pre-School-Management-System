    ...
    <form action="{{route('credential')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="col-md-3 mb-3">
                    ...
                    <select name="userId" class="form-control">
                        @foreach($data as $row)
                            <option value="{{$row->t_id}}">{{$row->t_id}}</option>
                        @endforeach
                    </select>

                </div>
            <div class="col-md-4 mb-4">
                <label for="formGroupExampleInput" class="form-label">Teacher UserName</label>
            ...
            <input type="hidden" class="form-control" id="formGroupExampleInput" name="userType" value="teacher"/>
        </div>
